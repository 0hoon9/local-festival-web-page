		...
		<div id="container_box">
			 ...
			 <hr>
		 	 ...
		 	 <div id="board">
		 	 	...
		 	 	<button type="button" id="selectDeleteBtn">
							...
							<tr>
								...
								<td>
									...
									<div id="content" class="content_${request.rnum}">
										...
										<br><c:out value="${request.content}"/>
										
										<c:if test="${request.grpord == 0}">
											<button type="button" id="response_${request.rnum}_btn">답변하기</button>
										</c:if>
										...
										<div id="response" class="response_${request.rnum}">
											...
											<textarea rows="4" cols="50" id="response_${request.rnum}_content"></textarea>
											
											<button type="button" id="submit_${request.rnum}_btn">등록</button>
											<button type="button" id="reset_${request.rnum}_btn">취소</button>
										</div>
										
										<c:if test="${request.grpord > 0}">
											<button type="button" id="update_${request.rnum}_btn">수정하기</button>
										</c:if>
										 ...
										 <div id="updateResponse" class="updateResponse_${request.rnum}">
										 	...
										 	<textarea rows="4" cols="50" id="updateResponse_${request.rnum}_content">${request.content}</textarea>
										 	
										 	<button type="button" id="updateSubmit_${request.rnum}_btn">수정</button>
											<button type="button" id="updateReset_${request.rnum}_btn">취소</button>
										 </div>
									...
									<script>
										...
										$(document).ready(function(){
											...
											$("#reset_${request.rnum}_btn").click(function(){
												$("#response_${request.rnum}_btn").css("display","block");
											...
											$("#update_${request.rnum}_btn").click(function(){
												...
												/* $("#updateResponse_${request.rnum}_content").val("${request.content}"); */
												$("#update_${request.rnum}_btn").css("display","none");
											})
											
											//취소버튼 클릭
											$("#updateReset_${request.rnum}_btn").click(function(){
												$("#update_${request.rnum}_btn").css("display","block");
