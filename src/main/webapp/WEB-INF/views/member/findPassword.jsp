...
<body>
	<h1>비밀번호 찾기</h1>
	<form role="form" action="/member/findPassword" method="post">
		아이디 입력: <input type="text" id="user_id"><br>
		이름 입력: <input type="text" id="user_Name"><br>
		<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
		<button id="modbtn" type="submit">수정하기</button><br>
	</form>
	<a href="/board/main">메인화면으로 돌아가기</a>
</body>
	...
	$.ajax({
		...
		data: {
			...
			success: function(data){
				if(data==false){
					alert("아이디와 이름이 맞지 않아요");
					} else {
						alert("아이디와 이름이 맞습니다. 비밀번호 변경 절차를 진행합니다");
